<?php
	...
	setcookie('GetYourAddress', 'Your cookie', time() + 3600);	//设置cookie
	require_once('IpLocation.class.php');		// 导入IpLocation类
	
	$ipclass = new IpLocation('qqwry.dat');
	...
	mysql_set_charset($db_config["charset"]);
	$table = 'ip';
	$keyword = $_POST['keyword'] ? $_POST['keyword'] : '';
	$sql = "insert into $table(ip, keyword, city, area, time) values('$ip', '$keyword', '{$area['country']}', '{$area['area']}', '$time')";
	$res = mysql_query($sql);
	mysql_close($link);
?>
<!DOCTYPE html>
...
<script>
	...
	function init() {
		...
		getMap(addr);
		console.log('Get Your Address : ' + addr);
	}
	
	//搜索
	function searchAddr() {
		var addr_f = document.getElementById('addr').value;		// 初始化搜索关键字
		$.post('/getip/getIP.php', { keyword: addr_f});
		if(addr_f == '') {
			var addr = addr_p != '' ? addr_p : '北京市';
		}else{
			var addr = addr_f;
